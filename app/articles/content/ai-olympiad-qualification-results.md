...
После этого о�� выбрал некоторый **порог** $T$ и оставил только те фотографии, для которых $P_i \ge T$. Он заметил, что при изменении порога $T$ точность (precision) может **уменьшаться** — то есть модель становится менее уверенной. Абдикожа решил найти все такие **значения порога $T$**, в которых функция $\text{PRECISION}(T)$ убывает.
...
В отличие от предыдущего задания, где вы разрабатывал�� feature extractor для компьютерного зрения, здесь вы будете отправлять **полный пайплайн**, включающий:
