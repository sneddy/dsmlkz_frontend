    ...
    try {
      ...
      if (signUpError) {
        ...
        if (
          ...
          // Отправляем на страницу с универсальным сообщением
          router.push(`/email-verification?email=${encodeURIComponent(email)}&type=existing`)
          return
        } else {
      ...
      setTimeout(() => {
        router.push(`/email-verification?email=${encodeURIComponent(email)}&type=new`)
      }, 1000)
    } catch (err: any) {
